...
function oah {
	...
	CMD_FOUND=""
	CMD_TARGET="${OPENAPPHACK_DIR}/src/oah-${COMMAND}.sh"
	if [[ -f "$CMD_TARGET" ]]; then
		CMD_FOUND="$CMD_TARGET"
	fi

	# Check if it is a sourced function
	CMD_TARGET="${OPENAPPHACK_DIR}/ext/oah-${COMMAND}.sh"
	if [[ -f "$CMD_TARGET" ]]; then
		CMD_FOUND="$CMD_TARGET"
	fi
	...
	if [ -n "$CMD_FOUND" ]; then
		...
		__oah_"$CONVERTED_CMD_NAME" "$QUALIFIER" "$3" "$4"
	fi

	# Attempt upgrade after all is done
	if [[ "$COMMAND" != "selfupdate" ]]; then
	    __oah_auto_update "$OPENAPPHACK_REMOTE_VERSION" "$OPENAPPHACK_VERSION"
	fi
}
